...
t_img	*get_img(t_rend *rend, char ch, int time)
{
	...
	if (ch == FLOOR_CH)
		return (&rend->floor);
	if (ch == HERO_CH && time < TICK * 1)
		return (&rend->hero0);
	if (ch == HERO_CH && time < TICK * 2)
		return (&rend->hero1);
	if (ch == HERO_CH && time < TICK * 3)
		return (&rend->hero2);
	if (ch == HERO_CH && time < TICK * 4)
		return (&rend->hero3);
	if (ch == HERO_CH && time < TICK * 5)
		return (&rend->hero4);
	if (ch == HERO_CH && time < TICK * 6)
		return (&rend->hero5);
	if (ch == COLL_CH)
